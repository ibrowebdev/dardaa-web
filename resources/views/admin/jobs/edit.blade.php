...
@section('title', 'Refine Vacancy')
@section('page_title', 'Refine Career Listing')
...
<div class="max-w-4xl">
    ...
    <form action="{{ route('admin.jobs.update', $job) }}" method="POST" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        @csrf
        @method('PUT')
            ...
            <div class="bg-[#1A1A24] border border-border/50 rounded-2xl p-6 sm:p-8 shadow-xl space-y-6">
                ...
                <div>
                    ...
                    <div class="flex rounded-xl overflow-hidden border border-border bg-[#0D0D14] focus-within:border-primary transition">
                        ...
                        <input type="text" name="slug" id="slug" required value="{{ old('slug', $job->slug) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="department" id="department" required value="{{ old('department', $job->department) }}"
                               class="w-full bg-[#0D0D14] border border-border focus:border-primary text-white rounded-xl px-4 py-3 outline-none transition text-sm">
                    </div>

                    <div>
                        <label for="location" class="block text-xs font-bold text-muted uppercase tracking-widest mb-2">Job Location *</label>
                        <input type="text" name="location" id="location" required value="{{ old('location', $job->location) }}"
                               class="w-full bg-[#0D0D14] border border-border focus:border-primary text-white rounded-xl px-4 py-3 outline-none transition text-sm">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="12" required
                              class="w-full bg-[#0D0D14] border border-border focus:border-primary text-white rounded-xl px-4 py-3 outline-none transition text-sm font-sans leading-relaxed">{{ old('description', $job->description) }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="bg-[#1A1A24] border border-border/50 rounded-2xl p-6 shadow-xl space-y-6">
                ...
                <div class="flex items-center justify-between pt-4 border-t border-border/50">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ $job->is_active ? 'checked' : '' }}>
