...
@section('title', 'Log Testimonial')
@section('page_title', 'Log Client Endorsement')
...
<div class="max-w-4xl">
    ...
    <form action="{{ route('admin.testimonials.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        @csrf
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="bg-[#1A1A24] border border-border/50 rounded-2xl p-6 shadow-xl text-center space-y-4">
                    ...
                    <div class="w-32 h-32 rounded-full border-2 border-dashed border-border hover:border-primary bg-[#0D0D14] mx-auto relative cursor-pointer overflow-hidden flex items-center justify-center transition group"
                        ...
                        <template x-if="!photoPreview">
                            <div class="text-center p-4">
                                <svg class="w-6 h-6 text-muted mx-auto mb-1 group-hover:text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                                <span class="block text-[10px] font-bold text-muted group-hover:text-white">Upload</span>
                            </div>
                        </template>
                ...
                <p class="text-[10px] text-muted/60">Provide an optional user headshot or corporate avatar.</p>
            </div>

            {{-- Rating Selector --}}
            <div class="bg-[#1A1A24] border border-border/50 rounded-2xl p-6 shadow-xl space-y-4" x-data="{ rating: {{ old('rating', 5) }} }">
                ...
                <div class="flex justify-center items-center gap-2">
                    ...
                    @for($i = 1; $i <= 5; $i++)
                        <button type="button" @click="rating = {{ $i }}" class="transition hover:scale-110 active:scale-95 cursor-pointer outline-none focus:outline-none">
            ...
            <div class="bg-[#1A1A24] border border-border/50 rounded-2xl p-6 sm:p-8 shadow-xl space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="name" id="name" required value="{{ old('name') }}" autofocus placeholder="e.g. Jonathan Doe"
                    ...
                    <div>
                        ...
                        <input type="text" name="company" id="company" required value="{{ old('company') }}" placeholder="e.g. Apex Global"
                               class="w-full bg-[#0D0D14] border border-border focus:border-primary text-white rounded-xl px-4 py-3 outline-none transition text-sm">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="role" id="role" value="{{ old('role') }}" placeholder="e.g. Technical Founder, CTO"
                           class="w-full bg-[#0D0D14] border border-border focus:border-primary text-white rounded-xl px-4 py-3 outline-none transition text-sm">
                </div>
                ...
                <div>
                    ...
                    <textarea name="quote" id="quote" required rows="5" placeholder="Insert the customer endorsement text verbatim..."
                              class="w-full bg-[#0D0D14] border border-border focus:border-primary text-white rounded-xl px-4 py-3 outline-none transition text-sm font-medium resize-none italic leading-relaxed">{{ old('quote') }}</textarea>
                </div>
            </div>
